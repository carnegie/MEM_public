...
def core_model(case_dic, tech_list):
    ...
    demandTot = 0.0 # keep track of total demand in case CO2 intensity is constrained
    if not 'constrain_co2_intensity' in case_dic: # True means constrain CO2/kWh average. False means just constrain total CO2.
        case_dic['constrain_co2_intensity'] = False
    ...
    for tech_dic in tech_list:
        ...
        #     adjusted based on delta_t
            ...
            if 'charging_time' in tech_dic:
                if efficiency > 0 and tech_dic['charging_time'] > 0:
                    constraints += [ dispatch_in  <= capacity / (efficiency * tech_dic['charging_time'] / delta_t) ]
                    constraint_list += [tech_name + ' dispatch_in_le_charging_rate']
            if 'discharge_time' in tech_dic:
                discharge_time = tech_dic['discharge_time']
            else:  # use charging time if discharge time not in tech_dic
                if 'charging_time' in tech_dic:
                    discharge_time = tech_dic['charging_time']
                else:
                    discharge_time = -1
            if discharge_time > 0:
                constraints += [ dispatch <= capacity / (discharge_time / delta_t) ]
    ...
    if case_dic['co2_constraint'] >= 0 and flag_emissions == True:
        if case_dic['constrain_co2_intensity']:
            constraints += [totCO2e <= case_dic['co2_constraint'] * demandTot ]
        else:
    ...
    gurobi_time = datetime.datetime.now()    # timer starts
    if verbose:
        print ('    gurobi time = ',gurobi_time)
        print ('    elapsed time start to gurobi = ',gurobi_time - start_time)
...
#    
    ...
    if verbose:
        ...
        print ('    elapsed time in gurobi = ',end_time - gurobi_time)
