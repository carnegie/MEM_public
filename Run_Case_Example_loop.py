...
for idx in range(len(tech_list)):
    ...
    if name == 'natgas_ccs': co2_emis_natgas = tech_list[idx]['var_co2']
...
### Set cycle values
co2_constraints_percentage = np.array([45]) #run for CCS
...
for idx in range(len(co2_constraints_list)):
    ...
    case_dic['case_name'] = case_name_default + '_Co2Cons' + str(co2_constraints_percentage[idx])
