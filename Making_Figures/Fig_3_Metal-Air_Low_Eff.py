...
import os
import sys
module_directory = '/Users/annali/Documents/MEM_public.nosync/Making_Figures/4Form'
...
def cost_conts(ax, path, x_labels, colors):
    ...
    for i, x_label in enumerate(x_labels):
        if x_label == '':
            ...
            batt_cost[i] = 0
            continue
        elif x_label == 'Metal-Air_Low_Eff':
...
storage_names = ['Li-ion', 'RFB', 'PSH', 'Gravitational', 'Thermal', 'CAES', 'Metal-Air_Low_Eff', 'Hydrogen']
storage_colors= [purple, green, darkblue, red, orange, lightblue, gray, pink] #For the cases with 1 tech

Li_ion_PGP_X_techs = ['RFB', 'PSH', 'Gravitational', 'Thermal', 'CAES', 'Metal-Air_Low_Eff'] #Name of folders containing pickle for each technology in case
PGP_X_techs = ['RFB', 'PSH', 'Gravitational', 'Thermal', 'CAES', 'Metal-Air_Low_Eff'] #Name of folders containing pickle for each technology in case
Li_ion_X_techs = ['RFB', 'PSH', 'Gravitational', 'Thermal', 'CAES', 'Metal-Air_Low_Eff', 'PGP'] #Name of folders containing pickle for each technology in case

storage_x_colors = [green, darkblue, red, orange, lightblue, gray, pink] #For all cases with >1 tech

# Just Storage X

X_directory = '/Users/annali/Documents/MEM_public.nosync/MEM_code/Output_Data/CONUS/One_Tech_X'

X_techs = ['Li-ion', 'RFB', 'PSH', 'Gravitational', 'Thermal', 'CAES', 'Metal-Air_Low_Eff', 'PGP'] #For the cases with 1 tech
...
#fig_w = 160*mm
fig_h = 100*mm
fig, axes = plt.subplots(nrows=1,ncols=30, figsize=(fig_w,fig_h), dpi=900)
...
ax.set_ylabel('System Cost Contributions ($/kWh)')
#ax.set_title('No\nStorage')
...
ax4 = plt.subplot2grid((1, 29), (0, 23), colspan=6, rowspan=1)
...
#ax.text(-0.11, 1.15, 'A', transform=ax.transAxes, fontsize=10, fontweight='bold')
...
#ax2.text(-0.15, 1.05, '10.9', transform=ax2.transAxes, fontsize=9)
#ax3.text(-0.28, 1.05, '10.8', transform=ax3.transAxes, fontsize=9)

plt.savefig('Metal-Air_Low_Eff.png', bbox_inches='tight')
#plt.show()
