...
inputs[4]: battery
inputs[5]: to_PGP
inputs[6]: PGP_storage
inputs[7]: from_PGP
inputs[8]: 3rd_tech_in_power
inputs[9]: 3rd_tech_energy
inputs[10]: 3rd_tech_out_power
tech_list[0]: demand
...
storage_tech = ['battery', 'to_PGP', 'PGP_storage', 'from_PGP', 'to_third_tech', 'third_tech_energy', 'from_third_tech'] # Both energy and power components
energy_tech = ['battery', 'PGP_storage', 'third_tech_energy'] # Energy components
parameters = ['fixed_cost', 'capacity', 'dispatch', 'stored']


def get_data_sep_power(path, techidx1, var1, techidx2, var2, nones):
    ...
    for i in range(len(unsorted_list)):
        ...
        results = base[1]
        #var, pickle
        par_list.append((inputs[techidx1][var1], inputs[techidx2][var2], base))
    ...
    for base in base_list:
        ...
        data['pgp_cost'].append(add(dic.get('to_PGP'), dic.get('PGP_storage'), dic.get('from_PGP')))
        data['third_tech_cost'].append(add(dic.get('third_tech_energy'), dic.get('in_third_tech'), dic.get('from_third_tech')))
        data['system_cost'].append(results[0].get('system_cost'))
        data['ng_cost'].append(dic.get('natgas'))
    ...
    data['third_tech_dur'] = calculate_duration(data, 'third_tech_energy', 'from_third_tech', nones)
    data['PGP_dur'] = calculate_duration(data, 'PGP_storage', 'from_PGP', nones)
    
    data['third_tech_cycles'] = calculate_cycles(data, 'from_third_tech', 'third_tech_energy', nones)
    data['PGP_cycles'] = calculate_cycles(data, 'from_PGP', 'PGP_storage', nones)
