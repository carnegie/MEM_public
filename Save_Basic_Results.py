...
def save_basic_results(case_dic, tech_list,constraint_list, constraints,prob_dic,capacity_dic,dispatch_dic,stored_dic):
    ...
    for tech_dic in tech_list:
        ...
        # dispatch_dic keys have ' dispatch' appended
        if tech_name + ' dispatch' in dispatch_dic:
            results_tech_dic[tech_name + ' dispatch'] = np.average(dispatch_dic[tech_name + ' dispatch'])
        # stored_dic keys have ' stored' appended
        if tech_name + ' stored' in stored_dic:
            results_tech_dic[tech_name + ' stored'] = np.average(stored_dic[tech_name + ' stored'])
    ...
    for node in node_price_dic:
        ...
        # Find associated energy demands and calculate mean energy weighted price
        # This assumes a single demand per node, which is not enforced in MEM
        for item in tech_list:
            if item['tech_type'] == 'demand' and item['node_from'] == node:
                results_case_dic[node+' mean demand'] = np.average(item['series'])
                meanDemand = results_case_dic[node+' mean demand']
                if meanDemand != 0.0:
                    results_case_dic[node+' energy weighted price'] = np.average(results_time_dic[node+' price'] * item['series']) / results_case_dic[node+' mean demand']
                else:
                    results_case_dic[node+' energy weighted price'] = 0.0                   
                results_time_dic[node+' price'] = results_time_dic[node+' price']
                tot += results_case_dic[node+' energy weighted price'] * results_case_dic[node+' mean demand']
                print("Added node:", node)
                break # ends for loop
    results_case_dic['price closure'] = results_case_dic['system_cost'] - tot
    #print(f"\nPRICE CLOSURE: {results_case_dic['price closure']}\n")
    ...
    # COMMENT OUT
    # Uncommenting this section will save dual values
    # for all vector type constraints. The current code (21 Sept 2021)
    # saves dual values for the price of energy at each node
    #
    vect = []
    name1 = ''
    for idx in range(len(constraints)):
        dual = constraints[idx].dual_value
        try:  # len fails if dual not list
            if len(dual) == num_time_periods:
                results_time_dic[constraint_list[idx]] = dual/input_case_dic['numerics_scaling']
        except:
            if 'storage_balance_step' in constraint_list[idx]:
                name = constraint_list[idx].split('_step')[0]
                if name == name1 or len(vect) == 0:
                    vect.append(dual/input_case_dic['numerics_scaling'])
                    name1 = name
                if len(vect) == num_time_periods:
                    results_time_dic[name] = vect
                    vect = []
                    print(f"Added {name}")
            else:
                print(f"{constraint_list[idx]},{dual/input_case_dic['numerics_scaling']}")
            pass
...
def compute_derived_values(args):
    ...
    for tech_dic in input_tech_list:
        ...
        if tech_type == 'generator':
            ...
            results_tech_dic[tech_name + ' curtailment'] = np.average(derived_time_dic[tech_name + ' curtailment'])
