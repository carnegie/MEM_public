...
def save_basic_results(case_dic, tech_list, constraints,prob_dic,capacity_dic,dispatch_dic,stored_dic):
    ...
    include_timeseries_results = True if 'include_timeseries_results' not in case_dic.keys() else case_dic['include_timeseries_results']
    ...
    # extra lost load values
    if 'lost_load dispatch' in dispatch_dic:
        results_tech_dic['lost_load max dispatch'] = np.max(dispatch_dic['lost_load dispatch'])
        results_tech_dic['lost_load num hours'] = (dispatch_dic['lost_load dispatch'] > 0).sum()
    ...
    # Allow turning off saving time series to save memory
    if not include_timeseries_results:
        input_time_dic = {}
        results_time_dic = {}
