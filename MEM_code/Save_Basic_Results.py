...
def save_basic_results(case_dic, tech_list, constraints,prob_dic,capacity_dic,dispatch_dic,stored_dic):
    ...
    results_time_dic_fixed_generators = {} # Enrico added

    num_time_periods = case_dic['num_time_periods']
    results_time_dic['time_index'] = np.array(range(num_time_periods))
    results_time_dic_fixed_generators['time_index'] = np.array(range(num_time_periods)) # Enrico added
    for item in tech_list:
        ...
        if 'series' in item:
            ...
            if item['tech_type'] != 'fixed_generator': # Enrico added
                results_time_dic_fixed_generators[tech_name + ' potential'] = item['series']*factor # Enrico added
    node_price_dic = prob_dic['node_price']
    for node in node_price_dic:
        results_time_dic[node+' price'] = node_price_dic[node]
        results_time_dic_fixed_generators[node+' price'] = node_price_dic[node] # Enrico added
    for item in dispatch_dic:
        results_time_dic[item] = dispatch_dic[item]
        results_time_dic_fixed_generators[item] = dispatch_dic[item] # Enrico added
    for item in stored_dic:
        results_time_dic[item] = stored_dic[item]
        results_time_dic_fixed_generators[item] = stored_dic[item] # Enrico added
    ...
    results_time_dic_fixed_generators.update(derived_time_dic) # Enrico added
    ...
    results_time_bis_df = pd.DataFrame(results_time_dic_fixed_generators)
    ...
    output_file_name_1 = case_name + '_(case_results)'
    output_file_path_name_1 = output_folder + "/" + output_file_name_1
    writer_1 = pd.ExcelWriter(output_file_path_name_1 + '.xlsx', engine = 'xlsxwriter')
    results_case_df.to_excel(writer_1, sheet_name = 'case results')
    writer_1.save()
    output_file_name_2 = case_name + '_(tech_results)'
    output_file_path_name_2 = output_folder + "/" + output_file_name_2
    writer_2 = pd.ExcelWriter(output_file_path_name_2 + '.xlsx', engine = 'xlsxwriter')
    results_tech_df.to_excel(writer_2, sheet_name = 'tech results')
    writer_2.save()
    output_file_name_3 = case_name + '_(time_results)'
    output_file_path_name_3 = output_folder + "/" + output_file_name_3
    writer_3 = pd.ExcelWriter(output_file_path_name_3 + '.xlsx', engine = 'xlsxwriter')
    results_time_bis_df.to_excel(writer_3, sheet_name = 'time results')
    writer_3.save()
